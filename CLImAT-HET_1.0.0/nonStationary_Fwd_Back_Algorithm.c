/* 
 nonStational Forward Backward HMM algorithm
 ...
 A             Position specific state transition probabilities matrix Pr(x_{k} = i| x_{k - 1} = j)
	...
	double *alpha, double *gamma, double *loglik, double *scale)
{
	...
	for (t = 1; t < K; t++) {
		...
		for (j = 0; j <N; j++) {
			sum = 0.0;
			for (i = 0; i <N; i++)
				sum += alpha[i+(t-1)*N]*A[i+j*N+(t-1)*N*N]; 
...
double *beta)	
{
        ...
        for (t = K - 2; t >= 0; t--) {
                ...
                for (i = 0; i <N; i++) {
                        beta[i+t*N] = 0.0;
                        for (j = 0; j <N; j++)
                                beta[i+t*N] += A[i+j*N+t*N*N]*L[j+(t+1)*N]*beta[j+(t+1)*N];
                        sum += beta[i+t*N];
                }
                for (i = 0; i <N; i++) 
                				beta[i+t*N] /= sum;
...
void ComputeXi(int N, int K, double *A, double *L, double *alpha, double *beta, double *gamma, double *tmp_xi, double *xi_summed)	
{
	...
	for (t = 0; t <= K - 2; t++) {
		...
		for (i = 0; i <N; i++) 
				for (j = 0; j <N; j++) {
					tmp_xi[i+j*N] = alpha[i+t*N]*A[i+j*N+t*N*N]*L[j+(t+1)*N]*beta[j+(t+1)*N];
					sum += tmp_xi[i+j*N];
				}
